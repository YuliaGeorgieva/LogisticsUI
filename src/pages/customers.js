...
const Page = () => {
  ...
  const [getCoustumerdata, setGetCoustumerdata] = useState([]);
  const [addCostumer, setAddCostumer] = useState({
    email: "",
    password: "",
    name: "",
    branchId: 0,
    ...
    axios({
      ...
      .then((response) => {
        if (response.status === 200) {
          setGetCoustumerdata(response.data);
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  const HandleCustomerData = (e) => {
    ...
    setAddCostumer({ ...addCostumer, [e.target.name]: SearchValues });
  };

  const HandleSubmitCustomer = (e) => {
    e.preventDefault();
    setAddCostumerData([...addCostumerData, addCostumer]);
    // axios({
    //   method: "POST",
    //   url: "https://localhost:7231/api/Customers/AddCustomer",
    //   data: addCostumerData[0]
    // })
    setShowAlert(true);
    axios({
      method: "POST",
  ...
  const handlerFilterCustomerdata = (e) => {
    const searchValue = e.target.value.toLowerCase(); // Convert the search value to lowercase for case-insensitive filtering

    setFilterData(searchValue);

    if (searchValue !== "") {
      const filteredCustomer = getCoustumerdata?.filter((item) => {
        return (
      ...
      setGetCoustumerdata(filteredCustomer);
    } else {
      // If the search input is empty, reset to the original data
      setGetCoustumerdata(getCoustumerdata);
      ShowAllCustomer();
    }
  };
      ...
      >
        <Container maxWidth="xl">
          <Stack spacing={3}>
            ...
            <Card sx={{ p: 2 }}>
              <OutlinedInput
                onChange={handlerFilterCustomerdata}
                defaultValue=""
                fullWidth
            ...
            <CustomersTable
              ShowAllCustomer={ShowAllCustomer}
              getCoustumerdata={getCoustumerdata}
              HandleCustomerData={HandleCustomerData}
              count={data.length}
              items={customers}
